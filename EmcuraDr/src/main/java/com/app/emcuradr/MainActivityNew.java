...
import com.app.emcuradr.adapter.CategoriesAdapter;
import com.app.emcuradr.api.ApiCallBack;
import com.app.emcuradr.api.ApiManager;
import com.app.emcuradr.model.CategoriesModel;
...
import com.app.emcuradr.reliance.therapist.ActivityReferedPatients;
import com.app.emcuradr.services.GPSTracker;
import com.app.emcuradr.util.ActionEditText;
import com.app.emcuradr.util.CallWebService;
...
public class MainActivityNew extends AppCompatActivity implements ApiCallBack, GloabalSocket.SocketEmitterCallBack{
	...
	TextView tvMessagesBadge,tvLiveCareBadge,tvWaitLiveCareBadge,tvAppointmentBadge,tvPrescriptionsBadge,tvRefillsBadge,
			tvGroupsBadge,tvAddDoctorBadge,tvSoapRefBadge,tvCovidBadge;
	...
	public void setBadge(Database database){//Activity activity
		...
		final ArrayList<String> waitingCares = database.getAllNotifByType(DATA.NOTIF_TYPE_CARE_REQ);
		...
		final ArrayList<String> covidRequests = database.getAllNotifByType(DATA.NOTIF_TYPE_COVID_REQ);
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if(tvWaitLiveCareBadge!=null){
					if(waitingCares!= null){
						DATA.print("--liveCares size "+waitingCares.size());
						if(waitingCares.size() > 0){
							tvWaitLiveCareBadge.setText(waitingCares.size() + "");
							//badge.setTextSize(12);
							tvWaitLiveCareBadge.setVisibility(View.VISIBLE);
						}else {
							tvWaitLiveCareBadge.setVisibility(View.GONE);
						}
					}else {
						tvWaitLiveCareBadge.setVisibility(View.GONE);
					}
				...
				if(tvCovidBadge!=null){
					if(covidRequests!= null){
						DATA.print("--covidRequests size "+covidRequests.size());
						if(covidRequests.size() > 0){
							tvCovidBadge.setText(covidRequests.size() + "");
							//badge.setTextSize(12);
							tvCovidBadge.setVisibility(View.VISIBLE);
						}else {
							tvCovidBadge.setVisibility(View.GONE);
						}
					}else {
						tvCovidBadge.setVisibility(View.GONE);
					}
	...
	protected void onResume() {
		...
		//DATA.selectedUserCallId = "";//due to barcode scan function
...
//		exportDB();
		...
		tvWaitingLiveCare = findViewById(R.id.tvWaitingLiveCare);
		...
		tvCovidList = findViewById(R.id.tvCovidList);
		...
		tvWaitLiveCareBadge = findViewById(R.id.tvWaitLiveCareBadge);
		...
		tvWaitingLiveCare.setOnClickListener(view ->
		{
			openActivity.open(ActivityCareRequestsList.class, false);
		});
...
//				openActivity.open(HomeHealth.class, false);
		...
		tvCovidList.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
...
//		};
		...
		DATA.allCategories.add(new CategoriesModel("20", "Scan Patient Barcode",R.drawable.ic_drawer_barcodescan));
		DATA.allCategories.add(new CategoriesModel("17", "Support",R.drawable.ic_drawer_support));
		DATA.allCategories.add(new CategoriesModel("20", "My Profile",R.drawable.ic_drawer_my_profile));
		DATA.allCategories.add(new CategoriesModel("21", "Logout",R.drawable.ic_drawer_logout));
	...
	private void parseLabelsData(String content) {
		try {
			...
			String chat_gpt_key = data.getString("chatgpt_key");
			String chatgptQueries = data.optString("chatgpt_queries");
			String disclaimer = data.optString("disclaimer");
			sharedPrefsHelper.save("disclaimer",disclaimer);
			DATA.print("allQ"+chatgptQueries);


			// Remove the outer square brackets
			String trimmedString = chatgptQueries.substring(1, chatgptQueries.length() - 1);
			...
			ArrayList<String> gptList = new ArrayList<>(Arrays.asList(stringArray));

			for(String s:gptList){
				DATA.print("once:"+s);
			}

			// save queries for later use

			sharedPrefsHelper.saveGptQueries(gptList);


			sharedPrefsHelper.save("gpt_key",chat_gpt_key);
...
//				setTitle(patientArry[position]);								
	...
	private void initZxing(){
		...
		//integrator.setCaptureActivity(CameraTestActivity.class);
		//integrator.setDesiredBarcodeFormats(IntentIntegrator.ONE_D_CODE_TYPES);
		integrator.setPrompt("Please scan your patient QR code / barcode");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(result != null) {
			if(result.getContents() == null) {
				...
				DATA.print("-- QR / Barcode Scaneed : "+ qrCode);

				DATA.selectedUserCallId = qrCode;
				DATA.selectedLiveCare = new MyAppointmentsModel();
				Intent intent = new Intent(activity, ActivityTcmDetails.class);
				intent.putExtra("isFromQRscan",  true);
				startActivity(intent);
