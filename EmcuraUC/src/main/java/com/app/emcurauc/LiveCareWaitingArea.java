            ...
            public void onClick(View v) {
                ...
                if (prefs.getString("docId_eLiveCare", "").equalsIgnoreCase("0")) {//means doc not assigned by OM
                    new AlertDialog.Builder(activity, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT)
                            .setMessage("Are you sure ? Do you want to cancel the care request ?")
                            .setPositiveButton("Yes Cancel", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    ...
                                    params.put("livecare_id" , DATA.liveCareIdForPayment);
...
//                        .create().show();
    ...
    Dialog dialogCancelReason;

    public void initCancelDialog() {
        dialogCancelReason = new Dialog(activity);
        dialogCancelReason.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialogCancelReason.setContentView(R.layout.dialog_cancel_reason);
        final EditText etMsg = (EditText) dialogCancelReason.findViewById(R.id.etCancelReason);
        TextView btnCancel = (TextView) dialogCancelReason.findViewById(R.id.btnCancel);
        TextView btnDismiss = (TextView) dialogCancelReason.findViewById(R.id.btnDismiss);

        btnCancel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View arg0) {

                String cancelReason = etMsg.getText().toString();
                if (!cancelReason.isEmpty()) {
                    RequestParams params = new RequestParams();
                    params.put("cancel_reason", etMsg.getText().toString());
                    ApiManager.shouldShowLoader = true;
                    ApiManager apiManager = new ApiManager(ApiManager.CANCEL_UC_REASON,"post",params,apiCallBack, activity);
                    apiManager.loadURL();
                }else{
                    customToast.showToast("Please enter cancel reason", 0 ,0);
                }

            }
        });

        btnDismiss.setOnClickListener(arg0 -> dialogCancelReason.dismiss());

        WindowManager.LayoutParams lp = new WindowManager.LayoutParams();
        lp.copyFrom(dialogCancelReason.getWindow().getAttributes());
        lp.width = WindowManager.LayoutParams.MATCH_PARENT;
        lp.height = WindowManager.LayoutParams.WRAP_CONTENT;
        dialogCancelReason.show();
        dialogCancelReason.getWindow().setAttributes(lp);
    }
    ...
    public void fetchDataCallback(String httpStatus, String apiName, String content) {
        ...
        if (apiName.contains(ApiManager.GET_WAITING_PATIENTS)) {
            ...
            try {
                ...
                if (status.equals("1") || status.equals("") || status.equals("2")) {
...
//                    avtTimeMillis = (yourNum * 600000);//300000 milis = 5 minutes, 600000 = 10 minutes
        ...
        else if(apiName.equalsIgnoreCase(ApiManager.CANCEL_UC_REASON)) {
            try {
                JSONObject jsonObject = new JSONObject(content);
                String status = jsonObject.optString("status");
                if (status.equalsIgnoreCase("success")) {
