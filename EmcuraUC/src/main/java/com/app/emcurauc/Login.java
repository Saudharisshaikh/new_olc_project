...
import com.app.emcurauc.permission.PermissionsActivity;
...
public class Login extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, ApiCallBack {
    ...
    public static String emailFromSocial, fnameFromSocial, lnameFromSocial, fromSocial, socialId, imageUrlFromSocial;
    ...
    protected void onResume() {
		...
		/*if(permissionsChecker.lacksPermissions(PermissionsChecker.PERMISSIONS)){
            ...
            if (NewSignup.is_signup_successfull) {
                NewSignup.is_signup_successfull = false;
                etLoginUsername.setText(NewSignup.signupUsername);
                etLoginPassword.setText(NewSignup.signupPassword);
...
/*		rgOptions.setOnCheckedChangeListener(new OnCheckedChangeListener() {
        ...
        btnAppUpdate.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://play.google.com/store/apps/details?id=com.app.emcurauc&hl=en_US"));
                startActivity(intent);
            }
        });
        ...
        btnLogin.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (etLoginUsername.getText().toString().isEmpty() || etLoginPassword.getText().toString().isEmpty()) {
                    ...
                    DATA.print("dToken"+device_token);
                    params.put("device_token", device_token);
                    params.put("platform", "android");
                    params.put("timezone", TimeZone.getDefault().getID());
                    params.put("hospital_id", prefs.getString("hospitalIdLogin", ""));
        ...
        if (apiName.equals(ApiManager.PATIENT_LOGIN)) {
            try {
                ...
                if (status.equals("success")) {
                    ...
                    if (user_info.getString("is_approved").equals("0")) {
                        loginFrom = "2";
                        SharedPreferences.Editor ed = prefs.edit();
                        ed.putString("id", user_info.getString("id"));
                        ed.putString("userEnteremail", user_info.getString("email"));
...
//							}
        ...
        } else if (apiName.equals(ApiManager.CHECK_SOCIAL)) {
            try {
                JSONObject jsonObject = new JSONObject(content);
                String status = jsonObject.getString("status");
                                ...
                                new GraphRequest.GraphJSONObjectCallback() {
                                    ...
                                    public void onCompleted(JSONObject object, GraphResponse response) {
                                        ...
                                        try {
                                            ...
                                            fnameFromSocial = fullName;
                                            fromSocial = "facebook";
                                            socialId = id;
                                            imageUrlFromSocial = imageUrl;
                                            checkSocial("facebook", id, prefs.getString("hospitalIdLogin", ""));
                                            //end
//Ahmer comment social Login code 11-07-22
										/*	RequestParams params = new RequestParams();
											params.put("gender", "_");
											params.put("social_id",id);
											params.put("email",email);
...
-- Request code: 64206 result code: -1 intentData: Intent { (has extras) }   facebook logged in*/
    ...
    private void handleSignInResult(GoogleSignInResult result) {
        try {
            ...
            DATA.print("--sign in result " + Objects.requireNonNull(result.getSignInAccount()).getEmail() + " ; " + result.getSignInAccount().getId()
            ...
            if (result.isSuccess()) {
                ...
                socialId = id;
                imageUrlFromSocial = imageUrl;
                checkSocial("google", id, prefs.getString("hospitalIdLogin", ""));
                //end
        ...
        client.post(RequestUrl, params, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    // versionCheck  by using API *Saud
                    checkAppVersion();
    ...
    public void checkSocial(String from_social, String socialid, String hosptialId) {
        RequestParams params = new RequestParams();
        params.put("hospital_id", hosptialId);
        params.put("social_id", socialid);
        params.put("from_social", from_social);

        ApiManager apiManager = new ApiManager(ApiManager.CHECK_SOCIAL, "post", params, apiCallBack, activity);
        apiManager.loadURL();
    }
    ...
    public void socialLogin() {
        ...
        params.put("picture", imageUrlFromSocial);
        params.put("first_name", fnameFromSocial);
        params.put("hospital_id", prefs.getString("hospitalIdLogin", ""));
        params.put("from_social", fromSocial);
        if (latitude == 0.0 || longitude == 0.0) {
            gps = new GPSTracker(activity);
            if (gps.canGetLocation()) {
                latitude = gps.getLatitude();
