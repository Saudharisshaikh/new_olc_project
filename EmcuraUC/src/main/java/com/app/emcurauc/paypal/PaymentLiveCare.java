...
/*		Intent intent = new Intent(this, PayPalService.class);
		...
		radioHealthInsurance.performClick();//to reset values to paypal
...
/*		if (requestCode == REQUEST_CODE_PAYMENT) {
   ...
   /* public void saveTransaction(
		...
		DATA.print("--params in saveTransection patient_id "+patient_id+" livecare_id "+livecare_id+" amount "+amount+" payment_method "+payment_method+" transaction_id "+transaction_id);
		pd.show();
		client.post(DATA.baseUrl+"/saveTransaction/"+patient_id,params ,new AsyncHttpResponseHandler() {
			@Override
			public void onSuccess(int statusCode, String content) {
				pd.dismiss();
				DATA.print("-- responce in saveTransaction "+content);
				...
				ed.putString("getLigetLiveCareApptID", DATA.liveCareIdForPayment+"");
				ed.commit();
			...
			public void onFailure(Throwable error, String content) {
				...
				DATA.print("--responce in failure getMedicalHistory: "+content);
		...
		stripe.createCardToken(cardParams, new ApiResultCallback<Token>() {
			...
			public void onSuccess(Token token) {
				...
				DATA.print("-- Token created: "+token.toString()
						...
						+"\ntoken bank account: "+token.getBankAccount()
						+"\ntoken card: "+token.getCard()
						+"\ntoken created date: "+token.getCreated()
						+"\ntoken live mode: "+token.getLivemode()
						+"\ntoken used: "+token.getUsed()
						+"\ntoken id: "+token.getId());
