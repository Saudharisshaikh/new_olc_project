...
import com.app.fivestardoc.adapter.SupportMessageAdapter;
import com.app.fivestardoc.adapter.VVReportImagesAdapter2;
import com.app.fivestardoc.api.ApiManager;
import com.app.fivestardoc.model.IcdCodeBean;
...
public class LiveCareDetails extends BaseActivity {
    ...
    TextView tvSelPtName, tvPtSymptom, tvPtCondition, tvPtDescription, tvicdCodesLiveCare, tvPain, tvPainSeverity, tvPainBodyPart, tvSymptomDetails,
    ...
    LinearLayout layReports, layVideoCall, layMsg, layNEMT, layRemoveQueue, layInstantConnect, layRevSymptoms,
    layCallHist, layPresHist;

    public static boolean fromCompletedCare = false; // hide remove queue button after call

    BroadcastReceiver cancelReqBroadCast;

    @Override
    protected void onStart() {
        registerReceiver(cancelReqBroadCast, new IntentFilter("cancelledAppointment"));
        super.onStart();
    }

    @Override
    protected void onStop() {
        unregisterReceiver(cancelReqBroadCast);
        super.onStop();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvicdCodesLiveCare = (TextView) findViewById(R.id.tvicdCodesLiveCare);
        ...
        layPresHist = findViewById(R.id.layPresHist);
        layCallHist = findViewById(R.id.layCallHist);
        ...
        layRevSymptoms = findViewById(R.id.layRevSymptoms);
        layInstantConnect = findViewById(R.id.layInstantConnect);

        //check condtion current pt or past pt -- Ahmer 31 - 8 - 22
        if (LiveCare.isFromCompletedLiveCare == true) {
            LiveCare.isFromCompletedLiveCare = false;
            layRemoveQueue.setVisibility(View.GONE);
            getPetientDetails(DATA.selectedUserCallId, DATA.selectedUserAppntID);
        } else {
            layRemoveQueue.setVisibility(View.VISIBLE);
            getLiveCareICDCodes();
            getPetientDetails(DATA.selectedUserCallId, "");
        }
        //end
            ...
            public void onClick(View v) {
                switch (v.getId()) {
                    ...
                    case R.id.layVideoCall:
                        if (DATA.selectedLiveCare.is_online.equals("1")) {
                            if (LiveCare.isFromCompletedLiveCare) {
                                fromCompletedCare = true;
                                DATA.isVideoCallFromLiveCare = true;
                                DATA.incomingCall = false;
                                Intent myIntent = new Intent(getBaseContext(), MainActivity.class);//SampleActivity.class
                                myIntent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
                    ...
                    case R.id.layRemoveQueue:
                        ...
                        break;
                    case R.id.layRevSymptoms:
                        Intent intentRev = new Intent(activity, ActivityAiSymptomsReview.class);
                    ...
                    case R.id.layInstantConnect:
                        ...
                        startActivity(intent);
                        break;
                        case R.id.layPresHist:
                        Intent intentPresHist = new Intent(activity,MedicalHistoryActivity.class);
                        startActivity(intentPresHist);
                        //go to med hist activity
                        break;
                    case R.id.layCallHist:
                        Intent intent1  = new Intent(activity, ActivityCallLogs.class);
                        intent1.putExtra("isFromPatientScreen", true);
        ...
        layRevSymptoms.setOnClickListener(onClickListener);
        layInstantConnect.setOnClickListener(onClickListener);
        layPresHist.setOnClickListener(onClickListener);
        layCallHist.setOnClickListener(onClickListener);

        cancelReqBroadCast = new BroadcastReceiver() {
    ...
    public void getPetientDetails(String userId, String liveCareId) {
        ApiManager apiManager = new ApiManager(ApiManager.PATIENT_DETAIL_URGENTCARE + "/" + userId + "/" + liveCareId, "get", null, this, activity);
    ...
    public void fetchDataCallback(String httpStatus, String apiName, String content) {
        ...
        if (apiName.contains(ApiManager.PATIENT_DETAIL_URGENTCARE)) {
            ...
            try {
                ...
                String patId = patient_data.getString("id");
                DATA.print(patId);
                SharedPrefsHelper.getInstance().save("patId",patId);
                ...
                if (is_smoke.equalsIgnoreCase("0")) {
                    ...
                    try {
                        smokeHowMuchPerDay = arr[2];
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    try {
                        smokeReadyToQuit = arr[3];
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
        ...
        else if (apiName.contains(ApiManager.GET_LIVE_CARE_ICD_CODES)) {
            try {
                JSONObject jsonObject = new JSONObject(content);
                StringBuilder stringBuilder = new StringBuilder();
                JSONArray dataArray = jsonObject.getJSONArray("data");

                ArrayList<IcdCodeBean> icdCodeBeans = gson.fromJson(dataArray.toString(), new TypeToken<ArrayList<IcdCodeBean>>() {}.getType());
                sharedPrefsHelper.saveIcdCodes(icdCodeBeans);
                ...
                tvicdCodesLiveCare.setText(stringBuilder.toString());

                sharedPrefsHelper.save("patient_icd_codes",stringBuilder.toString());
                sharedPrefsHelper.save("patient_ID_ICD",DATA.selectedUserCallId);
            } catch (JSONException e) {
                e.printStackTrace();
            }
        }
    ...
    public void getLiveCareICDCodes() {
        RequestParams params = new RequestParams();
        params.put("id", DATA.selectedLiveCare.liveCheckupId);
        ApiManager apiManager = new ApiManager(ApiManager.GET_LIVE_CARE_ICD_CODES, "post", params, apiCallBack, activity);
        apiManager.loadURL();
    }
}
