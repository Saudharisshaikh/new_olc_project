...
public class PresscriptionActivity extends AppCompatActivity implements ApiCallBack {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		findViewById(R.id.tabOptions).setVisibility(View.GONE);
		...
		if(apiName.equalsIgnoreCase(ApiManager.GET_PRESCRIPTIONS)){
			try {
				...
				for (int i = 0; i < data.length(); i++) {
					...
					prescriptionBean = new PrescriptionBean(id,status,reqType,patient_id, doctor_id, vitals, diagnosis, treatment, dateof, first_name, last_name, image,signature,
							ppStatus);
