...
package com.app.fivestardoc.fcm;
...
import com.app.fivestardoc.ActivityRifills;
import com.app.fivestardoc.ActivityTelemedicineServices;
import com.app.fivestardoc.BuildConfig;
import com.app.fivestardoc.ConversationsActivity;
import com.app.fivestardoc.DrsAppointments;
import com.app.fivestardoc.LiveCare;
import com.app.fivestardoc.LiveCareDetails;
import com.app.fivestardoc.Login;
import com.app.fivestardoc.MainActivityNew;
import com.app.fivestardoc.OnlineCareDr;
import com.app.fivestardoc.PresscriptionsActivity;
import com.app.fivestardoc.R;
import com.app.fivestardoc.ShowCancelPopup;
import com.app.fivestardoc.ShowGroupMessagePopup;
import com.app.fivestardoc.ShowLiveCarePopup;
import com.app.fivestardoc.ShowMessagePopupActivity;
import com.app.fivestardoc.SupportMessagesActivity;
import com.app.fivestardoc.VCallModule;
import com.app.fivestardoc.ViewConversationActivity;
import com.app.fivestardoc.model.ConversationBean;
import com.app.fivestardoc.model.SupportMessageBean;
import com.app.fivestardoc.services.IncomingCallResponse;
import com.app.fivestardoc.util.DATA;
import com.app.fivestardoc.util.DialogPatientInfo;
import com.app.fivestardoc.util.EasyAES;
        ...
        try {
            ...
            if (jsonObject.has("response_type") && jsonObject.getString("response_type").equalsIgnoreCase("message")) {
                ...
                if(DATA.shouldNotify){
                    ...
                    Intent i =new Intent().setPackage(getPackageName());
            ...
            if (status.equalsIgnoreCase("connected")) {
            ...
            else if(status.equals("new_care_patient")){
                String msg = jsonObject.getJSONObject("data").optString("message");
                String title = jsonObject.getJSONObject("data").optString("title");
                new DATA().insertNotification(context,DATA.NOTIF_TYPE_CARE_REQ);
                Intent intent1 = new Intent().setPackage(getPackageName());
                intent1.setAction(ActivityCareRequestsList.pt_added);
                getApplicationContext().sendBroadcast(intent1);
               // sendNotification(msg, ActivityCareRequestsList.class);
                showCustSoundNotification(msg, ActivityCareRequestsList.class);
                DATA.print("-- broadcast sent remove_patient ActivityCareRequestsList");
            }
            else if(status.equals("remove_patient")){
                Intent intent1 = new Intent().setPackage(getPackageName());
                intent1.setAction(ActivityCareRequestsList.pt_removed);
                getApplicationContext().sendBroadcast(intent1);
                DATA.print("-- broadcast sent remove_patient ActivityCareRequestsList");
               ...
               // sendNotification(msg, ActivityCareRequestsList.class);
                ...
                String pName = jsonObject.getJSONObject("data").getJSONObject("patient_data").getString("first_name")+" "+
                        jsonObject.getJSONObject("data").getJSONObject("patient_data").getString("last_name");

                String patient_id = jsonObject.getJSONObject("data").getJSONObject("patient_data").getString("id");
                String pImage = jsonObject.getJSONObject("data").getJSONObject("patient_data").getString("image");

                String cancelReason = jsonObject.getJSONObject("data").getString("cancel_reason");

                sendNotification(pName+" cancelled the care request cancel reason " + cancelReason, MainActivityNew.class);

                if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                    if(OnlineCareDr.isAppInForground){
                        Intent i =new Intent(context, ShowCancelPopup.class);
                        i.putExtra("msg", pName+ " cancelled the care request cancel reason " + cancelReason);
                        i.putExtra("pID", patient_id);
            ...
            else if (status.equalsIgnoreCase("new_result_recieved"))
            {
                String msg = jsonObject.getString("message");
                sendNotification(msg, ActivityLabRequests.class);
            }
            ...
            }else if(status.equalsIgnoreCase("new_covid_test")){
...
//    }
    ...
    private void showCustSoundNotification(String msg, final Class<? extends Activity> activityToOpen) {
        ...
        /*if (message.getNotification().getChannelId() == NOTIFICATION_EMERGENCY_CHANNEL_ID) {
            ...
            SOUND_URI = Uri.parse("android.resource://" + getApplicationContext().getPackageName() + "/" + R.raw.emergency_alarm);
        } else {
        ...
        String channelId = "GmtCC";

        Uri SOUND_URI = Uri.parse("android.resource://" + getApplicationContext().getPackageName() + "/" + R.raw.incoming_call);
